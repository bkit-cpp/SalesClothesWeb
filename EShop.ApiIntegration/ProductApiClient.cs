    ...
    public class ProductApiClient : IProductApiClient
    {
        ...
        public async Task<ApiResult<PagedResult<ProductVm>>> GetPagings(GetManageProductPagingRequest request)
        {
            try
            {
                ...
                var path = $"/api/product/all?pageIndex={request.PageIndex}" + $"&pageSize={request.PageSize}" + $"&keyword={request.Keyword}&languageId={request.LanguageId}&categoryId={request.CategoryId}";
        ...
        public async Task<bool> UpdateProduct( ProductUpdateRequest request)
        {
            try
            {
                ...
                var json = JsonConvert.SerializeObject(request);
                var path = $"/api/product/{request.Id}";
                var secretKey = _configuration["SecretKey"];
                var generateToken = HMACSHA256(path, secretKey);
                var languageId = "vi";
                var requestContent = new MultipartFormDataContent();
        ...
        public async Task<List<ProductVm>> GetFeaturedProducts(string languageId, int take)
        {
            ...
            try
            {
                ...
                client.BaseAddress = new Uri(_configuration["ServerHostIP"]);
                var secretKey = _configuration["SecretKey"];
                var path = $"/api/product/featured/{languageId}/{take}";
                var generateToken = HMACSHA256(path, secretKey);
                client.DefaultRequestHeaders.Add("Token", generateToken);
                var response = await client.GetAsync(path);
                var body = await response.Content.ReadAsStringAsync();
        ...
        public async Task<List<ProductVm>> GetLatestProducts(string languageId, int take)
        {
            try
            {
                ...
                client.BaseAddress = new Uri(_configuration["ServerHostIP"]);
                var secretKey = _configuration["SecretKey"];
                var path = $"/api/product/latest/{languageId}/{take}";
                var generateToken = HMACSHA256(path, secretKey);
                client.DefaultRequestHeaders.Add("Token", generateToken);
                var response = await client.GetAsync(path);
                var body = await response.Content.ReadAsStringAsync();
        ...
        public async Task<ApiResult<ProductVm>> GetById(int id, string languageId)
        {
            try
            {
                ...
                client.BaseAddress = new Uri(_configuration["ServerHostIP"]);
                var secretKey = _configuration["SecretKey"];
                var path = $"/api/product/{id}/{languageId}";
                var generateToken = HMACSHA256(path, secretKey);
                client.DefaultRequestHeaders.Add("Token", generateToken);
                var response = await client.GetAsync(path);
                var body = await response.Content.ReadAsStringAsync();
                var data = JsonConvert.DeserializeObject
                    <ApiSuccessResult<ProductVm>>(body);
                return data;
            }
            catch(Exception ex)
            {
